...
// Turn price string (querying raw price with WooGraphQl doesn't work yet) into float
...
const getSizeFromVariationInCart = (node) => {
  try {
    ...
    const sizeNode = attributes.find((attribute) => attribute.name === 'pa_size');
...
export const countryCodeToName = (countryCode) => {
  return countryList.find((country) => country.code === countryCode).name;
};
